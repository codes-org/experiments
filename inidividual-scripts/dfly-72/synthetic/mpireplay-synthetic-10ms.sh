...
mpirun_do() {
    ...
    mpirun --map-by :OVERSUBSCRIBE -np $np "${@:2}" > model-result.txt 2> model-result.stderr.txt
...
cat "$CONFIG_DIR"/dragonfly-72.template.conf.in | envsubst > terminal-dragonfly-72.conf

# configuration file for hybrid-lite and hybrid codes
#export IGNORE_UNTIL=0
#export SWITCH_TIMESTAMPS='"300", ".99e6"'
#end=1.001e6
export IGNORE_UNTIL=.00e6
export SWITCH_TIMESTAMPS='".05e6", ".8e6"'
end=1.001e6
export NETWORK_TREATMENT=freeze
export PREDICTOR_TYPE=average
cat "$CONFIG_DIR"/dragonfly-72-surrogate.template.conf.in | envsubst > terminal-dragonfly-72-hybrid.conf

# configuration file for hybrid-lite
export NETWORK_TREATMENT=nothing
cat "$CONFIG_DIR"/dragonfly-72-surrogate.template.conf.in | envsubst > terminal-dragonfly-72-hybrid-lite.conf
...
END
   ...
   mpirun_do high-fidelity "${simulation_params[@]}" \
          ...
          -- "$expfolder/terminal-dragonfly-72.conf"

   mpirun_do hybrid "${simulation_params[@]}" \
          --workload_conf_file="$subexp_folder/$work_alloc_file" \
          -- "$expfolder/terminal-dragonfly-72-hybrid.conf"
   
   #mpirun_do hybrid-lite "${simulation_params[@]}" \
   #       --workload_conf_file="$subexp_folder/$work_alloc_file" \
   #       -- "$expfolder/terminal-dragonfly-72-hybrid-lite.conf"
